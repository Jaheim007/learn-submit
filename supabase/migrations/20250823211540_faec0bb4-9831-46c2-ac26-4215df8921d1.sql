...
CREATE POLICY "Admins can insert supervisor records"
  ON public.supervisors
  FOR INSERT
  WITH CHECK (public.is_admin() OR auth.role() = 'service_role');
...
ADD CONSTRAINT IF NOT EXISTS user_roles_user_role_unique 
UNIQUE (user_id, role);

-- Clean up any admin emails that might be in students table
-- (This will be a manual cleanup - admins should not be students)
DO $$
BEGIN
  -- We'll let the application handle cleanup to avoid data loss
  RAISE NOTICE 'Migration completed. Admin and supervisor tables created with proper RLS.';
END
$$;