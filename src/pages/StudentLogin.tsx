...
export default function StudentLogin() {
  ...
  const handleGithubSignIn = async () => {
    try {
      ...
      if (error) {
        toast({ title: "Erreur", description: "Impossible de se connecter avec GitHub", variant: "destructive" });
      }
    } catch (error) {
        ...
        <CardHeader className="text-center">
          <Link to="/" className="inline-block">
            <img 
              src={hacktualzLogo} 
              alt="Hacktualiz Logo" 
              className="h-16 w-16 object-cover rounded-xl mx-auto mb-4 filter drop-shadow-[0_0_20px_hsl(350,85%,50%)] cursor-pointer hover:scale-110 transition-transform"
            />
          </Link>
        ...
        <CardContent>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input id="email" type="email" placeholder="votre.email@example.com" value={email} onChange={(e) => setEmail(e.target.value)} required disabled={loading} />
            </div>
            <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Button type="button" variant="outline" onClick={handleGithubSignIn} disabled={loading} className="h-11">
          ...
          <p className="mt-6 text-center text-sm text-muted-foreground">
            Pas de compte?{' '}
            <Link to="/etudiant/register" className="text-primary hover:text-primary-light font-medium transition-colors">S'inscrire</Link>
          </p>
        </CardContent>
      </Card>
    </div>
